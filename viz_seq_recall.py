    ...
    for i in sequences:
        ...
        for file,c in zip(files,colors):
            ...
            if 'ScanContext' == modelname:
                ax.plot(df['top'][3:25],df['recall'][3:25],color = c,label=modelname ,linewidth=5,linestyle = '--')
            else:
                #ax.plot(df.index[:25],df['recall'][:25],color = c,label=modelname ,linewidth=5,linestyle = '--')
                ax.plot(df.index[1:26],df['recall_rr'][1:26],color = c,label=modelname + '_ReR',linewidth=2,linestyle = '-')
                ax.plot(df.index[1:26],df['recall'][1:26],color = c,label=modelname ,linewidth=2,linestyle = '--')
