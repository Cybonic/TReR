...
if __name__ == '__main__':
    ...
    for i in sequences:
        ...
        fig, ax = plt.subplots()
        files = np.array(files_struct[i])

        idx = [int(file.split('/')[7].split('_')[2][1]) for file in files]
        sort_idx = np.argsort(idx)
        for file,c in zip(files[sort_idx],colors):
            ...
            if 'ScanContext' == modelname:
                ax.plot(df['top'][1:25],df['recall'][2:25],color = c,label=modelname ,linewidth=5,linestyle = '--')
            else:
                #ax.plot(df.index[:25],df['recall'][:25],color = c,label=modelname ,linewidth=5,linestyle = '--')
                ax.plot(df.index[2:25],df['recall_rr'][2:25],color = c,label= modelname[1] + " Encoder",linewidth=2,linestyle = '-')
        
        ax.plot(df.index[2:25],df['recall'][2:25],color = "darkorchid",label= "Baseline"  ,linewidth=2,linestyle = '--')
