...
class MetLocEvaluatorDemo():
    ...
    def evaluate(self,sequnce):
        ...
        # map_embeddings = descriptors[idx] for idx in database]

        #local_map_embeddings_keypoints = np.array([features[idx] for idx in database])
        #local_map_embeddings_features = np.array([keypts[idx] for idx in database])

        
        local_query_embeddings = [features[idx] for idx in queries]
        local_query_keypts =  [keypts[idx] for idx in queries]

        #map_positions = np.array([poses[idx] for idx in database]).reshape(-1,3)# Nmap x 2
